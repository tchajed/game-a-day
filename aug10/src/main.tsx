...
function App(){
  ...
  return <main className="desktop">
    ...
    <Browser page={page} data={data} setData={setData} time={time} account={account} setAccount={setAccount} setPage={setPage} field={field} proceed={proceed} selectedPhoto={selectedPhoto} setActive={setActive}/>
    {active==='passwords'&&<Passwords copy={copy}/>} 
    {active==='mail'&&<Mail copy={copy}/>} 
    {active==='photos'&&<Photos selected={selectedPhoto} choose={()=>{setSelectedPhoto(true);setNotice('visa-photo.jpg ready to upload');setTimeout(()=>setNotice(''),1600)}}/>}
    <nav className="dock">
      {(['browser','passwords','mail','photos'] as AppName[]).map(a=><button key={a} className={active===a?'active':''} onClick={()=>setActive(a)}><Icon name={a}/><i>{a==='passwords'?'Vault':a[0].toUpperCase()+a.slice(1)}</i></button>)}
    </nav>
...
function Browser({page,data,setData,time,account,setAccount,setPage,field,proceed,selectedPhoto,setActive}:any){
 return <section className="browser window">
  <div className="titlebar"><div className="lights"><i/><i/><i/></div><span>Travel Authorization — Navigator</span></div>
