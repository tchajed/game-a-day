...
const basePlaces: Place[] = [
  ...
  { id: 'rabbit', x: 78, y: 22, title: "Rabbit's Even Chance", subtitle: 'WIN PAYS 2×', kind: 'stall' },
...
class BadBetScene extends Phaser.Scene {
  ...
  private renderStall(game: GameKey) {
    ...
    this.add.text(width / 2, this.top + 22, fox ? 'THE SILVER SPIN' : "RABBIT'S EVEN CHANCE", {
    ...
    const consoleH = this.state.ledger ? (compact ? 250 : 220) : (compact ? 205 : 164)
    const consoleY = height - consoleH - 12
    ...
    this.add.text(width / 2, consoleY + 15, this.state.result, {
      fontFamily: FONTS.body, fontSize: `${clamp(width / 55, 19, 25)}px`, color: '#fff4cf', align: 'center', wordWrap: { width: width * .82 },
    }).setOrigin(.5, 0).setDepth(31)

    const amounts = [1, 5, 10, 25, 50]
    const buttonW = compact ? 46 : 58
    const gap = compact ? 6 : 8
    const amountY = consoleY + (compact ? 96 : 105)
    const amountStart = compact ? width / 2 - ((buttonW + gap) * amounts.length - gap) / 2 + buttonW / 2 : width * .13
    if (!compact) this.add.text(width * .055, amountY, 'BET', {
      fontFamily: FONTS.ui, fontSize: '16px', fontStyle: 'bold', color: '#dfb654', letterSpacing: 1,
    }).setOrigin(0, .5).setDepth(31)
    amounts.forEach((amount, index) => this.button(amountStart + index * (buttonW + gap), amountY, buttonW, 40, money(amount), () => {
      this.state.bet = amount
      this.renderMode()
    }, { fill: this.state.bet === amount ? COLORS.gold : COLORS.ink, color: this.state.bet === amount ? '#21182f' : '#fff1c7', stroke: 0x9c7d58, font: 15, depth: 32, disabled: amount > this.state.balance }))

    const playY = consoleY + (compact ? 148 : 105)
    const playX = compact ? width / 2 - 66 : width * .72
    this.button(playX, playY, 132, 44, fox ? 'SPIN ONCE' : 'TOSS ONCE', () => this.playBets(game, 1), { fill: COLORS.red, font: 16, depth: 32, disabled: this.state.balance < this.state.bet })
    this.button(playX + 96, playY, 52, 44, '×10', () => this.playBets(game, 10), { fill: COLORS.ink, stroke: 0x9c7d58, font: 15, depth: 32, disabled: this.state.balance < this.state.bet })
    this.button(playX + 156, playY, 52, 44, '×25', () => this.playBets(game, 25), { fill: COLORS.ink, stroke: 0x9c7d58, font: 15, depth: 32, disabled: this.state.balance < this.state.bet })
    if (this.state.ledger) this.renderLedger(game, consoleY + (compact ? 182 : 145))
  }
  ...
  private playBets(game: GameKey, count: number) {
    ...
    if (!played) {
      ...
      this.state.result = played === 1
        ...
        : `${played} PLAYS  ·  ${wins} WINS  ·  ${net >= 0 ? '+' : ''}${money(net)}`
    }
  ...
  private renderShop(type: 'ledger' | 'portrait' | 'tonic') {
    const data = {
      ledger: { icon: '▦', title: 'Practical Ledgers', keeper: 'OWL, SOLE PROPRIETOR', cost: 18, copy: 'Tracks wagers, wins, and return.', buy: 'BUY LEDGER  ·  $18' },
      portrait: { icon: '◒', title: 'Moon Portraits', keeper: 'LIKENESSES WHILE-U-WAIT', cost: 25, copy: 'A handsome souvenir. No strategic value.', buy: 'SIT FOR PORTRAIT  ·  $25' },
      tonic: { icon: '⚗', title: "Dr. Stoat's Tonic", keeper: 'FORMULATED WITH CONFIDENCE', cost: 30, copy: 'Sparkling confidence. Same odds.', buy: 'DRINK TONIC  ·  $30' },
    }[type]
