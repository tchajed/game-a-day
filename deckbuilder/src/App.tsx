...
function AlienPortrait({ encounter, speaking = true }: { encounter: Encounter; speaking?: boolean }) {
  ...
  return (
    <div className={`portrait portrait-${variant} ${speaking ? 'speaking' : ''}`} aria-label={`Hologram of ${encounter.envoy}`}>
      ...
      <div className="portrait-tag"><span>LIVE XENOLINK</span><i /></div>
    </div>
  )
}
...
function App() {
  ...
  return (
    <div className="app-shell">
      <header>
        <button className="brand" onClick={() => setView('mission')}><span className="brand-mark"><Satellite /></span><span><b>NULL</b> PROTOCOL<small>LOOP DIPLOMACY COMMAND</small></span></button>
        <nav>
          <button className={view === 'mission' ? 'active' : ''} onClick={() => setView('mission')}><Bot /> Manual Link</button>
          <button className={view === 'workshop' ? 'active' : ''} onClick={() => setView('workshop')}><LibraryBig /> Deck Workshop</button>
          <button className={view === 'archive' ? 'active' : ''} onClick={() => setView('archive')}><Archive /> Archive <span>{Math.min(3, game.encounterIndex + (game.intel >= 2 ? 1 : 0))}</span></button>
        </nav>
        <div className="header-tools">
          <div className="loop-chip"><Clock3 /><span>LOOP <b>{String(loop).padStart(2, '0')}</b></span></div>
      ...
      {view === 'mission' && <main className="mission-view">
        <aside className="mission-rail">
          <div className="eyebrow"><span /> Contact sequence</div>
          <h2>THE BRIDGE<br />CONVERSATIONS</h2>
          <p>Three talks. Infinite attempts. Your archive survives.</p>
          <div className="mission-list">
            {missionSteps.map((step, index) => <div key={step.id} className={`mission-step ${step.status}`}>
              ...
              <div><small>{step.status === 'done' ? 'ACCORD REACHED' : step.status === 'active' ? 'LINK ACTIVE' : 'PENDING'}</small><strong>{step.envoy}</strong><span>{step.location}</span></div>
              {step.status === 'locked' && <LockKeyhole />}
            </div>)}
          </div>
          <div className="automation-card"><Code2 /><div><span>AUTOMATION CORE</span><strong>OFFLINE</strong><small>Manual validation required</small></div><LockKeyhole /></div>
        </aside>

        <section className="contact-stage">
          <div className="stage-head">
            <div><div className="eyebrow"><span /> incoming delegation</div><h1>{encounter.envoy}</h1><p>{encounter.designation} <i>•</i> {encounter.location}</p></div>
            <div className={`affiliation ${factionKnown ? 'known' : ''}`}><small>AFFILIATION MODEL</small><strong>{factionKnown ? (encounter.faction === 'resonant' ? 'RESONANT' : 'EXACT') : 'UNRESOLVED'}</strong><span>{factionKnown ? 'HIGH CONFIDENCE' : `${game.intel}/2 INSIGHTS`}</span></div>
          </div>
          <div className="contact-body">
            ...
            <div className="transcript-panel">
              <div className="panel-title"><Terminal /><span>LIVE TRANSCRIPT</span><i>REC</i></div>
              <div className="transcript">
                <p className="intro-copy">{encounter.intro}</p>
                {game.logs.map((log, index) => <div className={`log ${log.kind}`} key={`${index}-${log.text}`}><span>{log.kind === 'signal' ? encounter.envoy.split(' ')[0] : log.kind === 'response' ? 'NEGOTIATOR' : log.kind === 'intel' ? 'ARCHIVE' : 'SYSTEM'}</span><p>{log.text}</p></div>)}
                <div ref={logsEndRef} />
              </div>
              <div className="intel-strip"><Archive /><span>Persistent insight</span><div>{[0, 1, 2].map(n => <i key={n} className={game.intel > n ? 'filled' : ''} />)}</div><b>{game.intel}</b></div>
            </div>
          </div>
        ...
        <aside className="hand-panel">
          <div className="hand-head"><div><span>MANUAL RESPONSE</span><h2>Choose a protocol</h2></div><div className="deck-count"><span>{game.drawPile.length}</span><small>DRAW<br />PILE</small></div></div>
          ...
          <div className={`response-preview ${selectedCard ? 'ready' : ''}`}>
            {selectedCard && preview ? <>
              ...
              <button className="transmit" onClick={() => playCard(selectedCard.id)}>TRANSMIT PROTOCOL <ChevronRight /></button>
            </> : <><Satellite /><p>Select one of the two drawn protocols.<br /><span>The other remains in hand.</span></p></>}
          </div>
          <div className="manual-note"><ShieldCheck /><span><b>MANUAL OVERRIDE</b> Every response is visible for validation.</span></div>
        </aside>
        ...
        {game.phase !== 'talk' && <div className="result-overlay">
          <div className={`result-modal ${game.phase}`}>
            {game.phase === 'reward' && <>
              ...
              <h2>A new protocol<br />survives the loop.</h2><p>Choose one response to add to your permanent archive. Rebuild your six-card deck before the next contact.</p>
            ...
            {game.phase === 'failed' && <>
              ...
              <button className="primary-action" onClick={restartTalk}><RotateCcw /> ENTER NEXT LOOP</button>
            ...
            {game.phase === 'complete' && <>
              ...
              <div className="final-stats"><div><b>{loop}</b><span>FINAL LOOP</span></div><div><b>{unlocked.length}</b><span>PROTOCOLS</span></div><div><b>3</b><span>ACCORDS</span></div></div>
      ...
      {view === 'workshop' && <main className="workshop-view">
        <section className="workshop-head"><div><div className="eyebrow"><span /> PERSISTENT LOADOUT</div><h1>DECK WORKSHOP</h1><p>Build a six-protocol algorithm for the next contact. Cards return every loop; knowledge does not.</p></div><div className={`deck-requirement ${deck.length === 6 ? 'valid' : ''}`}><span>{deck.length}</span><div><b>/ 6 PROTOCOLS</b><small>{deck.length === 6 ? 'LOADOUT VALID' : 'SELECT SIX TO DEPLOY'}</small></div></div></section>
        <section className="deck-workspace">
          <div className="active-deck"><div className="section-title"><div><small>ACTIVE LOADOUT</small><h2>Negotiator 01</h2></div><button onClick={() => setDeck(startingDeck)}><RotateCcw /> RESET</button></div>
            ...
            <button className="deploy-button" disabled={deck.length !== 6} onClick={applyDeck}><Bot /> DEPLOY MANUAL NEGOTIATOR <ChevronRight /></button>
          </div>
          <div className="library"><div className="section-title"><div><small>PERSISTENT ARCHIVE</small><h2>Protocol Library</h2></div><span className="owned-count">{unlocked.length} / {allCardIds.length} RECOVERED</span></div>
            <div className="library-grid">{allCardIds.map(id => unlocked.includes(id) ? <div className={`library-item ${deck.includes(id) ? 'in-deck' : ''}`} key={id}><CardView card={cards[id]} compact onClick={() => toggleDeckCard(id)} /><span className="deck-tag">{deck.includes(id) ? 'IN DECK' : deck.length < 6 ? '+ ADD' : 'DECK FULL'}</span></div> : <div className="locked-card" key={id}><LockKeyhole /><strong>UNKNOWN PROTOCOL</strong><span>Reach accord to recover</span></div>)}</div>
          </div>
        </section>
      </main>}

      {view === 'archive' && <main className="archive-view">
        <div className="archive-header"><div className="eyebrow"><span /> LOOP-PERSISTENT INTELLIGENCE</div><h1>CONTACT ARCHIVE</h1><p>Observations recovered by your negotiators across discarded timelines.</p></div>
